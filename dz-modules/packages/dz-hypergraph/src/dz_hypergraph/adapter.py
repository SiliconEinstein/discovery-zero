"""Zero HyperGraph <-> Gaia BP v2 adapter.

Converts Zero's HyperGraph into Gaia's FactorGraph, runs BP v2 inference, and
writes posterior beliefs back to the Zero graph.
"""
...
from gaia.bp import BPDiagnostics, BPResult, BeliefPropagation, FactorGraph, FactorType
...
class ZeroInferenceGraph:
    ...
    node_id_to_var_id: dict[str, str] = field(default_factory=dict)
    var_id_to_node_id: dict[str, str] = field(default_factory=dict)
    edge_id_to_factor_idx: dict[str, str] = field(default_factory=dict)
...
def adapt_zero_graph(
    ...
    *,
    ...
    for nid, node in graph.nodes.items():
        ...
        zig.node_id_to_var_id[nid] = nid
        zig.var_id_to_node_id[nid] = nid
    ...
    for eid, edge in graph.edges.items():
        ...
        if not premise_var_ids:
            zig.excluded_edge_ids.add(eid)
            continue

        p1_raw = float(edge.confidence)
        if edge.edge_type == "formal":
            p1_raw = max(p1_raw, 1.0 - CROMWELL_EPS)
        p2 = 1.0 - CROMWELL_EPS if edge.edge_type == "formal" else 0.5
        p1 = max(p1_raw, 1.0 - p2 + CROMWELL_EPS)

        factor_id = str(factor_idx)
        if len(premise_var_ids) == 1:
            fg.add_factor(
                factor_id=factor_id,
                factor_type=FactorType.SOFT_ENTAILMENT,
                variables=premise_var_ids,
                conclusion=conclusion_var_id,
                p1=p1,
                p2=p2,
            )
        else:
            ...
            zig.synthetic_var_ids.add(mediator)
            fg.add_factor(
                factor_id=f"{factor_id}_conj",
        ...
        zig.edge_id_to_factor_idx[eid] = factor_id
        factor_idx += 1

    # Contradiction factors: encoded via explicit relation variables.
    for eid_a, eid_b, cid in _detect_contradictions(graph):
        ...
        gate_var = f"contra_rel_{eid_a}_{eid_b}_{cid}"
        if cid not in zig.node_id_to_var_id:
            continue
        fg.add_variable(gate_var, 1.0 - CROMWELL_EPS)
        zig.synthetic_var_ids.add(gate_var)
        all_premises = list(set(a_premises + b_premises))
        if len(all_premises) < 2:
            continue
        fg.add_factor(
            factor_id=str(factor_idx),
            factor_type=FactorType.CONTRADICTION,
            variables=all_premises[:2],
            conclusion=gate_var,
        )
        zig.edge_id_to_factor_idx[f"_contra_{eid_a}_{eid_b}"] = str(factor_idx)
        factor_idx += 1

    # Equivalence factors: symmetric belief coupling via relation variables.
    for nid_a, nid_b in _detect_equivalences(graph):
        ...
        zig.synthetic_var_ids.add(relation_var)
        fg.add_factor(
            factor_id=str(factor_idx),
            factor_type=FactorType.EQUIVALENCE,
            variables=[var_a, var_b],
            conclusion=relation_var,
        )
        zig.edge_id_to_factor_idx[f"_equiv_{nid_a}_{nid_b}"] = str(factor_idx)
        factor_idx += 1

    validation_errors = fg.validate()
    if validation_errors:
        raise RuntimeError(
            "FactorGraph validation failed after adapt_zero_graph: "
...
def run_gaia_bp(
    ...
    *,
    ...
    try:
        bp_result: BPResult = bp.run(zig.factor_graph)
    except Exception as exc:
        logger.error(
            "BP failed on graph with %d nodes, %d edges: %s. "
            "Returning current beliefs unchanged.",
            len(graph.nodes),
            len(graph.edges),
            exc,
        )
    ...
    for var_id, belief in bp_result.beliefs.items():
        if var_id in zig.synthetic_var_ids:
            continue
        nid = zig.var_id_to_node_id.get(var_id)
        if nid is not None:
            node_beliefs[nid] = belief

    diag = bp_result.diagnostics if diagnostics else None
    converged = bp_result.diagnostics.converged
    iterations = bp_result.diagnostics.iterations_run
