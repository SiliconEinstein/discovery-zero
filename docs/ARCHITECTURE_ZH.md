...
节点 (Node)
  ...
  ├── prior: float            # 先验（MaxEnt 默认 0.5，符合 Gaia/Jaynes 理论）
...
- **同结论边去重**：前提集 Jaccard 相似度 ≥ 0.7 时合并为单因子，取最大 confidence；不同前提集的独立证据保持独立因子
...
| `DISCOVERY_ZERO_UNVERIFIED_CLAIM_PRIOR` | 0.5 | 未验证节点默认先验（MaxEnt） |
| `DISCOVERY_ZERO_JUDGE_MODEL` | （空） | 独立 Judge 模型（构造/验证分离） |
